...
const updatePage = async () => {
  ...
  const tmp = fruitsArray[0]
  // console.log(tmp["name"]);

  var paragraph = document.getElementById("sugar");
  if (paragraph) {
    paragraph.innerHTML = '';
}
  document.getElementById("sugar").textContent += "Fruits with sugar > 10g: ";


  if (Array.isArray(fruitsArray)) {
    fruitsArray.forEach(element => {
      const nutritions = element["nutritions"];
      const sugar = nutritions["sugar"];
      if (sugar > 10) {
  ...
  //   const te = element["nutritions"]
  //   console.log(te["sugar"]);
  // });



  // const filtered = Object.fromEntries(Object.entries(fruitsArray).filter(([key]) => key.includes('name')));
